...
- dashboard reorder-suggestions: REAL / BUSINESS-VALIDATED
- compare-series: REAL / SEMANTICALLY-ACCEPTED
- future-windows-stats: REAL / BUSINESS-VALIDATED
- entity-summary: REAL / STRUCTURE-VALIDATED (tree-only)
- planner/ops support endpoints: accepted safe stubs
- live gb_v2 stack untouched
- shadow stack untouched

## Priority order
1. reorder-suggestions ✅ deterministic edge cases validated
2. compare-series
3. future-windows-stats ✅ real non-empty validation completed
4. entity-summary ✅ accepted as tree-only hierarchical endpoint for current phase; quantitative totals explicitly deferred
5. dashboard-kpis
...
1. validate reorder-suggestions with edge cases
   ...
   - forecast low
   - forecast high
2. validate /api/v1/analytics/compare-series with real non-empty output ✅ multi-family + multi-fascia validated; current semantics accepted: famiglia may expose multiple rows per date
