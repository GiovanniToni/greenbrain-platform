...
def main():
    ...
    log_dir = Path("/opt/greenbrain-platform/runtime-reports")
    ...
    try:
        ...
        fam_file = Path("/opt/greenbrain-platform/runtime-reports/tmp/families.txt")
        fam_file.parent.mkdir(parents=True, exist_ok=True)
        fam_file.write_text("\n".join(fams) + ("\n" if fams else ""), encoding="utf-8")

        cmd = ["/bin/bash", "-lc", f"/opt/greenbrain-platform/apps/ml-worker/jobs/run_train_all_parallel.sh {JOBS}"]
        ...
        with logfile.open("w", encoding="utf-8") as logf:
            ...
            if not fams:
                ...
                child = subprocess.Popen(
                    ...
                    env=env,
                    cwd="/opt/greenbrain-platform/apps/ml-worker",
                    bufsize=1,
                )
