...
DEFAULT_HORIZON_DAYS = int(os.getenv("ETS_HORIZON_DAYS", "10"))
SEASONAL_PERIODS = 7
...
def _bundle_path(family_slug: str) -> str:
    return os.path.join(MODELS_DIR, f"bundle_{family_slug}_ets.pkl")
...
def _fit_forecasts(series: pd.Series, horizon: int) -> list:
    ...
    try:
        fit = ExponentialSmoothing(series, **kwargs).fit(optimized=True)
        raw = fit.forecast(horizon).clip(lower=0.0).tolist()
...
def predict_ets(
    ...
    try:
        ...
        if write_db:
            ...
            print(f"ENGINE_ETS rows_written={rows_written}")
    finally:
        db_engine.dispose()
