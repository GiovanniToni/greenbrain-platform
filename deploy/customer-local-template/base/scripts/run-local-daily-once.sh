...
docker compose -f docker-compose.local.yml --env-file "$ENV" run --rm --user root ml-worker \
  bash -lc "mkdir -p /workspace/overlay/logs/ml /workspace/overlay/ml/models_v4 /workspace/overlay/ml/storage && chown -R ${LOCAL_UID}:${LOCAL_GID} /workspace/overlay/logs /workspace/overlay/ml"
